...
- **Provenance — LOCAL-DECISION-GRADE (not yet archival):** this batch was run **locally in Claude Code, CPU, `--wandb-mode disabled`** (no W&B run). These numbers are trustworthy enough to *make decisions on and draft against* (frozen seeds, deterministic, committed CSVs). The **pre-submission archival-verification step** is a Colab re-run of the identical configs — `python -m Training.run_phase2_sweep --output-dir /content/drive/MyDrive/arch-ablations --wandb-mode online` (`--include-hold` for cell 5) — which, on the same frozen `MAIN_SEEDS`, must reproduce these numbers and produces the W&B-logged, Drive-archived copies of record. **The paper's final tables cite the archival Colab run; until then these are local-decision-grade.** (Same applies to the p=0.7 cell 5 once its local run lands.)
- **Config(s):** `configs/p2_{strict,alias,flicker08,proxnoise}_sym.yaml` — SYMMETRIC `[256,256]`, Exp 1 verbatim; only obs-wrapper differs. Symmetric-only = a statement about the TASK (ceiling/fracture for a standard agent), not an architecture comparison.
- **Raw per-seed CSVs:** `results/csv/p2_{strict,alias,flicker08,proxnoise}_sym.csv`.
- **Generated by:** `Training/run_phase2_sweep.py` (40-run batch).
