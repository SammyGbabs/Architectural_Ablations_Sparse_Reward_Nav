...
- **Provenance — LOCAL-DECISION-GRADE (not yet archival):** this batch was run **locally in Claude Code, CPU, `--wandb-mode disabled`** (no W&B run). These numbers are trustworthy enough to *make decisions on and draft against* (frozen seeds, deterministic, committed CSVs). The **pre-submission archival-verification step** is a Colab re-run of the identical configs — `python -m Training.run_phase2_sweep --output-dir /content/drive/MyDrive/arch-ablations --wandb-mode online` (`--include-hold` for cell 5) — which, on the same frozen `MAIN_SEEDS`, must reproduce these numbers and produces the W&B-logged, Drive-archived copies of record. **The paper's final tables cite the archival Colab run; until then these are local-decision-grade.** (Includes the p=0.7 cell 5, now landed — same local-decision-grade status, same Colab archival step pending.)
