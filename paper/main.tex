...
% NOTE (to finalise before an actual arXiv upload): author two's full given name
% and the exact affiliation split (African Leadership University / Ruzivo Research
% Lab) and the active-repo URL are placeholders derived from repository metadata —
...
our own earlier, unpublished work --- that an \emph{inverted} actor--critic asymmetry (a deeper actor
...
$P(\text{inverted} > \text{symmetric}) = 0.25$), and the ${\approx}12.5\times$ PPO sample-efficiency advantage we had earlier
...
--- four degradation mechanisms, each a dated amendment --- then shows \emph{why} the asymmetry
question cannot be settled here: no manipulation places the task in the regime the
hypothesis needs. Every rung either \textbf{ceilings} (the optimal policy stays easy to
represent) or \textbf{fractures} into a give-up local optimum (hard, but not in the way the
hypothesis specifies). We report this as a worked \emph{negative outcome of the protocol}, not
...
multi-seed re-evaluation \citep{henderson2018matters,agarwal2021precipice}; the accepted remedy is to
...
applies that lens critically to machine-learning benchmarks (\S\ref{sec:related}). That literature is largely
diagnostic, identifying validity failures in benchmarks after the fact; our contribution is a
constructive counterpart, an executable test a researcher can run \emph{before} trusting a
comparison.
...
($\pi=[512,256,128]$ vs $v=[256,128]$) --- appeared to outperform a symmetric baseline at matched
budget on a sparse-reward indoor-navigation task, on a single seed, together with an apparent
${\approx}12.5\times$ sample-efficiency advantage of PPO over DQN. Both claims come apart on re-examination --- but
\emph{how} they come apart is the more useful observation, and is what pointed us at the gap
above: one claim asked a question the task could not answer, the other a question the
measurement could not answer.

This paper makes two contributions. \textbf{First, a reusable protocol} (\S\ref{sec:protocol}) for deciding whether
...
\textbf{Second, a worked negative outcome}: applied to the navigation task, the protocol shows
the reported asymmetry advantage does not survive multiple seeds (\S\ref{sec:phase1}) and that \emph{no}
pre-registered degradation of the task (\S\ref{sec:phase2}) induces the regime the hypothesis needs --- the
task stays reactively, redundantly, memorably easy, or fractures the wrong way (\S\ref{sec:why}).
Throughout we are careful that this is a statement about \emph{the task}, not the hypothesis: we
show this task class cannot test inverted asymmetry, and specify (\S\ref{sec:need}) what a task that could
...
estimates \citep{agarwal2021precipice}. We adopt this machinery throughout. Our concern, however,
is orthogonal to it: these methods establish whether a measured difference is \emph{statistically}
...
\paragraph{Construct validity and benchmark critique.} That benchmarks may not measure what they are
taken to measure is not a new observation. Under the banner of \emph{construct validity} --- a
concept originating in psychological measurement theory, which asks whether a test captures
the phenomenon it claims to \citep{cronbach1955construct}, and recently imported into
machine-learning evaluation critique \citep{raji2021everything} --- a growing literature examines the
epistemic foundations of ML evaluation. Recent work develops explicit conditions of construct
validity for predictive benchmarking \citep{freiesleben2025benchmarking} and, through large-scale
systematic review, documents pervasive validity weaknesses across hundreds of benchmarks
\citep{bean2025measuring}. This literature is, however, almost entirely \emph{diagnostic}: it reviews
...
\emph{before} trusting a particular comparison, to establish whether a specific task can test a
...
\paragraph{Asymmetric actor--critic architectures.} The assumption that actor and critic should share
topology and capacity has been questioned from more than one direction. A recent line studies the
\emph{small-actor} regime, finding that shrinking the actor relative to the critic tends to degrade
performance through value underestimation and poor data collection, and that critic-side
optimism can recover it \citep{mastikhina2025optimistic}. The hypothesis we examine concerns the
\emph{opposite} direction --- a policy network deeper than its value network (``policy-hard,
...
effects is established on tasks that contain the relevant structure: \citet{mastikhina2025optimistic} report
clear effects on the control benchmarks they study. Their result is therefore a useful counterpoint to
ours --- an instance of a task that \emph{can} express a capacity-asymmetry effect --- and sharpens
...
\paragraph{Partial observability and task difficulty.} Our manipulation ladder degrades observability
...
a POMDP by stochastically obscuring observations \citep{hausknecht2015drqn} --- which we combine
with frame-stacking \citep{mnih2015human} so that any resulting hardness is representable by a
...
unless temporal structure is deliberately injected \citep{morad2023popgym}, a caution our results
bear out directly.

\paragraph{Pre-registration in machine learning.} Pre-registration --- fixing hypotheses, protocol, and
interpretation criteria before observing results --- is long established in medicine and
psychology, where registered reports are an accepted publication model \citep{chambers2013registered}, but
has seen comparatively little uptake in machine learning. Dedicated efforts to introduce it
into machine learning include the NeurIPS Pre-registration Workshops \citep{preregistration2020,preregistration2021},
whose motivation --- that an incentive structure rewarding only positive
...
\section{Phase 1 --- the reported effects do not survive, and their absence is not yet an answer}
\label{sec:phase1}
...
stratified-bootstrap confidence intervals over many seeds \citep{agarwal2021precipice} ---
...
concrete symptom of the gap \S\ref{sec:phase2} then diagnoses.

\paragraph{Setup.} The environment is a $20\times20$ residential grid (\texttt{Discrete(5)} actions, a
...
\citep{schulman2017ppo} hyperparameters (verbatim from those earlier experiments' configurations) for a fixed 200k-step
budget, at \textbf{10 seeds each}, and aggregate with rliable.
...
eval return \textbf{27.56} (inverted) versus \textbf{27.66} (symmetric), with overlapping 95\%
confidence intervals and a probability of improvement $P(\text{inverted} > \text{symmetric}) =
0.25$ --- i.e.\ the inverted configuration is, if anything, \emph{less} likely to beat the
symmetric one on a random seed. The single-seed advantage was seed noise;
the pre-registered falsification criterion for the asymmetry hypothesis (H1) is met.

\paragraph{Result 2 --- the sample-efficiency claim was the wrong kind of measurement.} Those same
earlier experiments also reported the policy-gradient agent (PPO) to be ${\approx}12.5\times$ more
sample-efficient than the value-based agent (DQN), on the basis that PPO reached stable high
performance in roughly 20 episodes against nearly 250 for DQN --- that is, $12.5\times = 250 / 20$,
a ratio of \emph{episodes-to-visual-convergence} read off single-run learning curves. This quantity is not a sound cross-algorithm sample-efficiency
...
${\sim}14$ steps --- so ``one episode'' purchases very different amounts of environment interaction
...
\emph{environment steps}. \textbf{(b) ``Stable high performance'' is eyeballed.} A convergence point
identified by eye has no defined threshold, and, read off a single curve, carries no
uncertainty. \textbf{(c) It is a single run} --- and \S\ref{sec:phase1}'s own H1 data shows how misleading single
runs are on this task, where one PPO seed collapses to a return of 3.6 while its nine
siblings sit at ${\sim}27.6$.

Re-measured with a defined, step-based, multi-seed statistic --- environment steps to reach
90\% of the asymptotic eval-return IQM, over 10 seeds, aggregated with rliable \citep{agarwal2021precipice} ---
no advantage remains; and to keep the comparison from turning on a chosen exemplar we
...
Exp 2 [14.0k, 74.0k]). The two families occupy the same ${\approx}20$--$45$k band; the single
configuration that leaves it, PPO Exp 3 at 163.3k steps, is the \emph{slowest} of either family ---
so on an exemplar-independent reading PPO is no faster than DQN, and the one salient
difference runs opposite to the claimed $12.5\times$ advantage.

These two results are one finding stated twice. The asymmetry claim put a question to a
\emph{task} that could not answer it --- a benchmark lacking the structure the hypothesis is about.
The sample-efficiency claim put a question to a \emph{measurement} that could not answer it --- a
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{p1_iqm_main_configs.png}
\caption{Per-configuration IQM eval return with 95\% stratified-bootstrap CIs across the
nine main configs (PPO Exp 1--4, DQN Exp 1--5), 10 seeds each.}
\label{fig:p1iqm}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.7\linewidth]{p1_perf_profile_ppo_inverted_vs_symmetric.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.7\linewidth]{p1_sample_efficiency_ppo_vs_dqn.png}
...
the \emph{task} could be unable to test it. Inspecting the observation makes the second
...
optimal \emph{policy} is a genuinely hard function while the \emph{value} stays smooth) predicts
\textbf{no} benefit from actor depth precisely when the policy is easy. On this observation
the precondition for the hypothesis is unmet \emph{by construction}, so a null is exactly
what the theory predicts --- and tells us nothing about the hypothesis itself. The
question this raises --- \emph{can this task test the hypothesis at all, under any
observation?} --- is what \S\ref{sec:phase2} answers with a pre-registered observability ladder, and
what \S\ref{sec:protocol} abstracts into a general protocol.

\section{Phase 2 --- a pre-registered observability ladder shows the task cannot test H1}
\label{sec:phase2}

\S\ref{sec:phase1} leaves a specific question: is the null a fact about the \emph{hypothesis} or about the
\emph{task}? To answer it we ask whether \textbf{any} degradation of the observation can put the
...
unlearnable. We answer this with a \textbf{pre-registered} experiment (the design,
...
The independent variable is a \emph{ladder} of observation wrappers over the frozen
...
would masquerade as the target regime, each rung is first \textbf{single-seed-gated for
...
model are design/calibration steps, never cited as results.
...
hard \citep{hausknecht2015drqn,morad2023popgym}), we test, over the frozen env: (i) \textbf{pure removal}
--- drop global position (A-MILD, 14-D) and additionally distance-to-target (A-STRICT,
13-D); (ii) \textbf{flicker $+$ frame-stack} --- each step the frame is fully masked with
probability $p$, then the last $k=4$ frames are stacked (52-D); (iii) \textbf{targeted
aliasing} --- drop the region one-hot so room/hallway/doorway cells are indistinguishable
from a single frame (10-D); (iv) \textbf{proximity noise} --- flip each binary proximity bit
with probability $q=0.3$ (13-D), degrading state-identification while leaving walls
mostly avoidable.

\paragraph{A calibration aside (why flicker rate is not the difficulty knob).} A
ground-truth BFS on the map gives optimal path lengths $H = \{\text{kitchen } 11, \text{bedroom } 11,
...
abandoned while bedroom is solved), so difficulty is \textbf{geometry/approach-dependent,
...
\begin{figure}[t]
\centering
\includegraphics[width=0.7\linewidth]{p2_rung3_blackout_vs_p.png}
\caption{$P(\text{disabling blackout} \geq 1 \text{ per episode})$ versus flicker probability $p$ at $k=4$,
...
\begin{figure}[t]
\centering
\includegraphics[width=0.7\linewidth]{p2_rung3_observed_vs_predicted.png}
...
or fractures; none is hard-but-learnable (Table~\ref{tab:ladder}; Figure~\ref{fig:ladder}):

\begin{table}[t]
\centering
\caption{Phase 2 claim-grade ladder cells --- symmetric PPO, 10 seeds, rliable IQM $+$ 95\% CI.}
\label{tab:ladder}
...
Flicker $p=0.8$ & 52-D & 200k & 8.95 [6.29, 13.01] & 0.00/1.00/0.33 & \textbf{fracture} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.7\linewidth]{p2_ladder_iqm.png}
\caption{The ladder cells of Table~\ref{tab:ladder} plotted: per-cell IQM eval return $+$ 95\% CI --- four
...
is absorbed, not lethal), and even the hardest \emph{learnable} flicker ($p=0.7$) reaches
the ceiling given budget --- 9/10 seeds at full success (the tenth, seed 9, plateaus
partway with bathroom SR 0.33; the IQM trims it, but we note it exists), the ${\sim}1.6$ gap
below 27.8 being a path-length tax from occasional blackouts, not sub-ceiling difficulty. \textbf{Second, the
one cell that leaves the ceiling does not become hard-but-learnable --- it fractures:}
at $p=0.8$, IQM collapses to 8.95 and the failure is a \emph{systematic give-up}, robust
across all 10 seeds --- \textbf{kitchen is abandoned in 10/10 seeds and bedroom solved in
10/10} (Figure~\ref{fig:fracture}), with failures being timeouts, not collisions.
That is a degenerate local optimum, not the graceful partial competence a fair
architecture test needs. \textbf{Third, this exhausts the axis:} across removal, flicker,
...
hypothesis is \textbf{untestable}: the null of \S\ref{sec:phase1} is a fact about the task, not (on this
evidence) about the hypothesis.

\begin{figure}[t]
\centering
\includegraphics[width=0.9\linewidth]{p2_flicker08_perseed.png}
\caption{The $p=0.8$ fracture, per seed --- left: outcome split (success / timeout /
...
actually \emph{contains} $S$. If the chosen benchmark lacks $S$, then $A$ has nothing to act
on and the hypothesis predicts \textbf{no} effect \emph{by construction} --- so a null result is
...
that the task is \emph{able} to test it. We distil our study into a reusable procedure for
that check. It is stated below in task- and hypothesis-agnostic terms; \S\S\ref{sec:phase1}--\ref{sec:phase2} are the
...
\item \textbf{Read the ladder --- and attribute the hardness.} If some rung is
   ...
   the policy). A rung can be hard the \emph{wrong} way: if it is, an architecture gap
   ...
   \textbf{on that rung}. If \textbf{no} rung reaches the precondition in a learnable,
   $S$-attributable regime, the task \emph{cannot} test the hypothesis. Report that, with the
...
contribution, not incidental lab habit. \textbf{Pre-registration} of the ladder, the
hypotheses, and --- critically --- a per-outcome \emph{interpretation table} fixed before any
...
meaning. \textbf{The gate/multi-seed split} is what makes exhaustive laddering affordable: a
...
it a detector, or merely a rejection stamp? To show it also returns \textbf{yes}, we run it as a
\textbf{positive control} on a task where the precondition can be inserted and removed at will,
recovering a \emph{known-true} effect rather than asserting a new one: $A =$ convolution,
$S =$ spatial locality. The ladder is a single fixed permutation of the MNIST pixels applied
to a fraction $q \in \{0, 0.25, 0.5, 0.75, 1.0\}$ of positions (nested subsets, a derangement
within each, identity elsewhere). Every rung is a \textbf{bijection} on pixel positions --- a fixed
relabeling of the input units --- so \textbf{information is provably preserved} (the classes remain
perfectly separable); what is progressively destroyed as $q \to 1$ is only spatial locality. We
...
\begin{figure}[t]
\centering
\includegraphics[width=0.7\linewidth]{pc_dose_response.png}
\caption{The dose-response of Table~\ref{tab:pc} plotted: CNN and MLP test accuracy vs scramble
...
Figure~\ref{fig:pc} shows the dose--response. The frozen predictions were: \textbf{P1}, the CNN degrades
monotonically as $q \to 1$; \textbf{P2}, the MLP stays flat (it is invariant to a fixed permutation
of its inputs); \textbf{P3}, the gap is large at $q=0$ and closes to ${\approx}0$ at $q=1$; \textbf{P4}, the
protocol therefore reports the task \emph{can} test $A$-suits-$S$ at $q=0$ and \emph{cannot} at $q=1$.
P1 held (CNN IQM $97.98 \to 93.18$, monotone). P4 held: at $q=0$ the CNN leads by \textbf{$+2.15$ points
with disjoint 95\% CIs} --- a resolvable effect --- while at $q=1$ there is no positive gap to
find.

\textbf{P3 was partially wrong, in an informative direction, and we report it as such.} The gap
does shrink monotonically, but at $q=1$ it does not settle at zero --- it crosses to $-2.52$,
the CNN landing \emph{below} the MLP with disjoint CIs. We had frozen ``${\approx}0$''; the data say the
convolutional prior is not merely uninformative on non-local inputs but mildly \emph{harmful} ---
...
each rung is a bijection, the MLP's \textbf{0.13-point} drift from $q=0$ to $q=1$ is optimization
noise, not information loss: the task did not get \emph{harder}, only locality was removed, so the
CNN's fall can only be the loss of the structure convolution exploits. (Flatness is judged by
a pre-committed absolute threshold, $|\Delta| < 1.0$ point, which holds comfortably at 0.13 points.
...
precision, not a real shift: with 10 seeds that interval is only ${\pm}0.1$ point wide, so a change
...
resolution --- pre-registration working as intended.) This is the \S\ref{sec:protocol} step-5 $S$-attribution guard
\textbf{succeeding} --- the complement of the navigation $p=0.8$ fracture, where the same guard
\emph{disqualified} a rung that had become hard the wrong way.
...
in, the protocol says \emph{yes}; on the navigation task of \S\S\ref{sec:phase1}--\ref{sec:phase2}, no manipulation could dial $S$
in at all, and it says \emph{no}. \textbf{The instrument works; the benchmark doesn't.}
...
\paragraph{Reactive optimality.} The optimal policy on this task appears to be well approximated by a
...
$p=0.5$ and 24\% at $p=0.7$ (\S\ref{sec:phase2}). Because a single recent frame suffices to act well, the agent
does not need to integrate across gaps; it needs only \emph{some} frame to be recent. Flickering
therefore attacks the \emph{availability} of the observation, not the \emph{complexity} of the function
...
(${\approx}0$), and the kitchen target is abandoned in 10 of 10 seeds while the bedroom is solved in 10
of 10. That is an optimisation pathology --- a give-up local optimum --- not a representational
demand, and it is precisely the case the $S$-attribution guard (\S\ref{sec:protocol}, step 5) exists to disqualify.

\paragraph{A single memorisable layout.} The map is fixed across all episodes, with optimal path
lengths of 11, 11, and 22 steps to the three targets (\S\ref{sec:phase2}). A policy can therefore encode
...
require held-out layouts of the kind \S\ref{sec:need} specifies.

\paragraph{Why this closes off the observation axis.} The three properties compound into a single
conclusion. The precondition the hypothesis requires --- a policy that is hard to represent
while the value function remains smooth --- is a property of the task's \emph{decision structure},
...
information \emph{scarcer} (flickering), \emph{narrower} (removal), or \emph{noisier} (corruption), but on
this task none of these made the underlying decision function \emph{more complex} --- its complexity
is set by the task's short-horizon, static, reactively-solvable structure, not by how the
observation is delivered. Hence the pattern across \S\ref{sec:phase2}: manipulations either
...
\item \textbf{Genuine, unroutable partial observability.} Hidden state that (i) cannot be
  ...
  ``reactive optimality'' and ``memorisable map'' escapes of \S\ref{sec:why}.)
\item \textbf{Non-redundant observation.} Each policy-relevant distinction is encoded once, so a
...
\item \textbf{A policy that is a sharp function even under full information.} Ideally the hardness
  is intrinsic to the optimal \emph{mapping} (high-frequency, many decision boundaries), not
...
and the architecture comparison of \S\ref{sec:phase1} could be run where it is meaningful. Whether the
...
terms of \S\ref{sec:related}, this operationalises the construct-validity question --- does this benchmark
measure what we take it to? --- as a prospective, pass/fail experiment run \emph{before} a comparison
...
and, by the mechanism of \S\ref{sec:why}, static single-map gridworld navigation with a redundant,
...
\textbf{not} shown that inverted actor--critic asymmetry fails to help in reinforcement
learning generally; the hypothesis may well hold on a task of the kind \S\ref{sec:need} specifies, and
...
\S\ref{sec:poscontrol} answers that objection directly. On a vision task where the precondition can be
inserted and removed at will, the same procedure returns a clean \emph{yes} where spatial
locality is present and \emph{no} where it is not, tracking the effect as it appears and
...
its negative branch (on RL navigation), but \textbf{not a substantive positive instantiation on
an RL task} --- one of the kind \S\ref{sec:need} specifies, where the asymmetry precondition is present in
a learnable regime and the architecture comparison can actually run. Constructing that task
and testing inverted asymmetry where it \emph{can} be tested is the natural next step; the
...
All quantitative claims in this paper are backed by committed per-seed data and
regenerable analysis. The supplementary material accompanying this submission contains:
(i) the per-seed raw CSVs behind every IQM and confidence interval (Phase 1: the nine
table configurations and three exploratory baselines at 10 and 5 seeds respectively;
Phase 2: the five claim-grade ladder cells at 10 seeds; and the positive-control
dose-response at 10 seeds $\times$ five scramble fractions $\times$ two architectures);
(ii) the analysis and plotting scripts that turn those CSVs into every figure and table,
using the rliable library for IQM aggregation with stratified-bootstrap confidence
intervals; (iii) the frozen pre-registration documents for both the observability ladder
and the positive control, including the dated amendment trail referenced in
\S\ref{sec:phase2}; and (iv) the environment and observation-wrapper code implementing the
frozen navigation task and the four degradation mechanisms of \S\ref{sec:phase2}.

The pre-registration documents fix, before any run, the hypotheses, the ladder, the
falsification criteria, and the per-outcome interpretation table on which the
methodological claim of this paper rests; they are included so that the ordering of
design decisions relative to results can be independently checked.
...
% rework repository (distinct from the frozen DLI-2026 submission repo).
\fi
