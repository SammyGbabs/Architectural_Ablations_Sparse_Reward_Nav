...
- **A2C is a "standard alternative" baseline, not a controlled ablation.** Only its **network architecture** is matched to the symmetric PPO Exp 1 (so it doesn't confound the asymmetry test); **every A2C algorithm hyperparameter is left at the SB3 A2C native default** (lr 7e-4, γ 0.99, gae_lambda 1.0, ent_coef 0.0, n_steps 5, Tanh activation) — *not* copied from PPO. So **PPO-vs-A2C differs in algorithm and most hyperparameters** — not a single-variable comparison. Double/Dueling DQN are matched to DQN Exp 5 (the best DQN config).
